...
def find_rows_with_last_col_one(tensor):
    """
    查找张量中最后一列元素为1.0的行号（1-based行号）
    
    参数:
    tensor (torch.Tensor): 输入的张量（形状为 [n, m]）
    
    返回:
    torch.Tensor: 满足条件的行号张量（1-based索引），若无匹配则返回空张量
    """
    # 提取最后一列
    last_column = tensor[:, -1]
    
    # 找到值为1.0的元素位置（0-based索引）
    row_indices_0based = torch.where(last_column == 1.0)[0]
    
    return row_indices_0based
...
class ResidualAttentionBlock_MaPLe(nn.Module):
    def __init__(self, d_model: int, n_head: int, attn_mask: torch.Tensor = None, design_details=None,
        ...
        self.attribute_len = design_details['attribute_len']
    ...
    def forward(self, inputs):
            ...
            if len(compound_prompts_deeper) > 0:
                ...
                if not self.text_layer:
                    ...
                    if not (counter > len(compound_prompts_deeper) - 1):
                        ...
                        textual_context = textual_context.expand(x.shape[1], -1, -1).half() # 50,4,512
                        
                        # print(f'textual_context is {textual_context.size()}') # 4,50,512

                        # index = find_row_indices_with_one(permutation)

                        # print(f'permutation is {permutation}')
                        # print(f'permutation size is {permutation.size()}') # 50,5,5

                        # print(f'index is {index}')
                        # anchor_1 = x[:attribute_len, :, :]

                        # permutation = permutation.permute(1,2,0) # 5,5,50 

                        # print(f"permutation is {permutation[0]}")
                        
                        row_numbers = find_rows_with_last_col_one(permutation[0])

                        if len(row_numbers) == 0:
                            permutation_modified = permutation[..., :-1] # 50,5,4
                            new_ctx = torch.matmul(permutation_modified, textual_context)  # 50,5,512
                            new_ctx = new_ctx.permute(1, 0, 2)  # 5,50,512
                        else:
                            anchor_index = row_numbers[0].item()
                            anchor_1 = x[anchor_index:anchor_index+1, :, :] # 1,50,512
                            anchor_1 = anchor_1.permute(1, 0, 2) # 50,1,512
                            new_ctx = torch.cat([textual_context, anchor_1], dim=1) # 50,5,512
                            # new_ctx = new_ctx.permute(1,0,2) # 50,5,512
                            new_ctx = torch.matmul(permutation, new_ctx)  # 50,5,512
                            new_ctx = new_ctx.permute(1, 0, 2)  # 5,50,512
                        
                        suffix = x[1 + attribute_len + self.compound_prompt_nctx:, :, :]

                        x = torch.cat([prefix, new_ctx, suffix], dim=0)
        ...
        return [x, compound_prompts_deeper, counter, permutation]  # return again as a list, so that nn.seq can work
...
class VisionTransformer_MaPLe(nn.Module):
    ...
    def forward(self, x: torch.Tensor, shared_ctx, compound_deeper_prompts, pos_matrix):
        x = self.conv1(x)  # shape = [*, width, grid, grid]
        x = x.reshape(x.shape[0], x.shape[1], -1)  # shape = [*, width, grid ** 2]
        x = x.permute(0, 2, 1)  # shape = [*, grid ** 2, width]
        x = torch.cat(
        ...
        outputs = self.transformer([x, compound_deeper_prompts, 0, pos_matrix])  # third argument is counter
        x = outputs[0]
        x = x.permute(1, 0, 2)  # LND -> NLD
