...
class NormalPromptLearner(nn.Module):
    def __init__(self, cfg, classnames, clip_model):
        ...
        self.ctx_pos_params = nn.Parameter(pos_params, requires_grad=False) # 16, 16
    ...
    def construct_prompts(self, ctx, prefix, suffix, anchor_1, permutation, label=None):
        ...
        ctx = torch.cat([ctx, anchor_1], dim=1)  # (dim0, n_ctx+anchor_len, ctx_dim)
        ctx = torch.matmul(permutation, ctx)  # (dim0, n_ctx+anchor_len, ctx_dim)
    ...
    def forward(self, im_features, anchor_1=None):
        ...
        permutation = F.gumbel_softmax(pos_params, tau=temp, hard=True)
        ...
        for ctx_shifted_i in ctx_shifted:
            ...
            pts_i = self.construct_prompts(ctx_i, prefix, suffix, anchor_1, permutation)  # (n_cls, n_tkn, ctx_dim)
...
class CustomCLIP(nn.Module):
    def __init__(self, cfg, classnames, clip_model):
        ...
        if cfg.DATASET.NAME == "Caltech101": # 97.93 95.42
            ...
            model_path = "anchanchor_weightsor/Food101/anchor_prompt_learner/model.pth.tar"
    ...
    def forward(self, image, label=None):
        ...
        normal_prompts = self.normal_prompt_learner(image_features, anchor_1)

        logits = []
        for pts_i, imf_i in zip(normal_prompts, image_features):
            text_features = self.text_encoder(pts_i, normal_tokenized_prompts)
...
class CoCoOp_AnchorOPT(TrainerX):
