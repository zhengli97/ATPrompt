...
class VLPromptLearner(nn.Module):
    def __init__(self, cfg, classnames, clip_model):
        ...
        if self.use_atp:
            ...
            if self.atp_num == 1:
                self.register_buffer("token_middle1", embedding[:, 1+n_att1 : 1+n_att1+1, :])  # attribute feature
                self.register_buffer("token_suffix", embedding[:, 1+n_att1+1+n_ctx :, :])  # CLS, EOS [102, 16:77,512]

            elif self.atp_num == 2:
                self.register_buffer("token_middle1", embedding[:, 1+n_att1 : 1+n_att1+1, :])  # attribute feature
                self.register_buffer("token_middle2", embedding[:, 1+n_att1+1+n_att2 : 1+n_att1+1+n_att2+1, :])  # attribute feature
                self.register_buffer("token_suffix", embedding[:, 1+n_att1+1+n_att2+1+n_ctx :, :])  # CLS, EOS [102, 16:77,512]

            elif self.atp_num == 3:
                self.register_buffer("token_middle1", embedding[:, 1+n_att1 : 1+n_att1+1, :])  # attribute feature
                self.register_buffer("token_middle2", embedding[:, 1+n_att1+1+n_att2 : 1+n_att1+1+n_att2+1, :])  # attribute feature
                self.register_buffer("token_middle3", embedding[:, 1+n_att1+1+n_att2+1+n_att3 : 1+n_att1+1+n_att2+1+n_att3+1, :])  # attribute feature
                self.register_buffer("token_suffix", embedding[:, 1+n_att1+1+n_att2+1+n_att3+1+n_ctx:, :])  # CLS, EOS [102, 16:77,512]
        else:
            self.register_buffer("token_suffix", embedding[:, 1 + n_ctx :, :])  # CLS, EOS
        ...
        if self.use_atp:
            if self.atp_num == 1:
                ...
                prompts = torch.cat(
                    [
                        prefix,  # (dim0, 1, dim)
                        ctx_att1, # (n_cls, n_att1, dim)
                        middle_attribute1,  # (n_cls, 1, dim)
                        ctx,  # (dim0, n_ctx, dim)
                        suffix,  # (dim0, *, dim)
                    ],
                    dim=1,
